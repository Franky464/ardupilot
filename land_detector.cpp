...
void Copter::update_land_detector()
{
    ...
    if (!motors->armed()) {
        ...
        float land_trigger_sec = g.land_detector_trigger_sec;
		//uint8_t land_detector_cond=g.land_detector_tasa;
        ...
        const bool WoW_check = true;
			...
			int32_t acceleration = land_accel_ef_filter.get().length(); // in order to be able to add it in the log
			int32_t hauteur = rangefinder_state.alt_cm_filt.get();
			int32_t vitesseZ = fabsf(inertial_nav.get_velocity_z_up_cms());
			int16_t gnd_clear = copter.rangefinder.ground_clearance_cm_orient(ROTATION_PITCH_270); //added bu Patrick..
			float mot_throttle =  motors->get_throttle_out();
			bool mot_low = mot_throttle <  motors->limit.throttle_lower + 0.05;
			bool hauteur_gnd_clear = hauteur < gnd_clear; // checking if height is lower than minimum with RNGFND
			
						AP::logger().Write("LNDT", "TimeUS,MotLo,TMM,AcSt,DeRat,RfCk,Tasa,HGC,Ldc","---------","00000000", "Qffffffff",
										AP_HAL::micros64(),
										(double)motor_at_lower_limit,
                                        (double)throttle_mix_at_min,
                                        (double)accel_stationary,
                                        (double)descent_rate_low,
                                        (double)rangefinder_check,	
                                        (double)g.land_detector_tasa,
										(double)hauteur_gnd_clear,
										(double)land_detector_count);
										
						AP::logger().Write("LNDV", "TimeUS,Acc,hight,Zspd,GndClr,ThO,Ldc","-omnm--","00BBB00", "Qffffff",
										AP_HAL::micros64(),
										(double)acceleration,
                                        (double)hauteur,
                                        (double)vitesseZ,
        ...
        if ((motor_at_lower_limit && accel_stationary && descent_rate_low && throttle_mix_at_min && rangefinder_check && WoW_check)|| // as previous 
		
			(mot_low && accel_stationary && descent_rate_low && throttle_mix_at_min && rangefinder_check && WoW_check && g.land_detector_tasa==1 && hauteur_gnd_clear)|| 	// no Mot_low_l						= AD
			(mot_low && descent_rate_low && throttle_mix_at_min && rangefinder_check && WoW_check && g.land_detector_tasa==3 && hauteur_gnd_clear)|| 						// no Mot_low_l,no Accel 			= D
			(motor_at_lower_limit && descent_rate_low && throttle_mix_at_min && rangefinder_check && WoW_check && g.land_detector_tasa==2 && hauteur_gnd_clear)|| 			// 				no Accel 			= MD
			(motor_at_lower_limit && throttle_mix_at_min && rangefinder_check && WoW_check && g.land_detector_tasa==6 && hauteur_gnd_clear)|| 								// 				no Accel,	no desc = M
			(mot_low && throttle_mix_at_min && rangefinder_check && WoW_check && g.land_detector_tasa==7 && hauteur_gnd_clear)|| 											// no Mot_low_l,no Accel,	no desc =
			(mot_low && accel_stationary && throttle_mix_at_min && rangefinder_check && WoW_check && g.land_detector_tasa==5 && hauteur_gnd_clear)|| 						// no Mot_low_l,			no desc = A
			(motor_at_lower_limit && accel_stationary && throttle_mix_at_min && rangefinder_check && WoW_check && g.land_detector_tasa==4 && hauteur_gnd_clear) 			// 							no desc = MA
